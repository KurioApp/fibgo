...
# Run e2e_test
DEP_IP=$(kubectl get service fibgo -o jsonpath='{.status.loadBalancer.ingress[0].ip}')
DEP_PORT=$(kubectl get service fibgo -o jsonpath='{.spec.ports[0].port}')
curl --fail http://${DEP_IP}:${DEP_PORT}/numbers || exit 1
